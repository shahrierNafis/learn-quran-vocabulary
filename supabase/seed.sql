...
--
alter table
  "public"."word_groups" enable row level security;
...
--
alter table
  "public"."user_intervals" enable row level security;
...
create table public.user_preference (
  ...
  constraint user_preference_uid_key unique (user_id),
  constraint user_preference_uid_fkey foreign key (user_id) references auth.users (id) on update cascade on delete cascade
) tablespace pg_default;

alter table
  "public"."user_preference" enable row level security;

create policy "Enable insert for users based on user_id" on "public"."user_preference" as PERMISSIVE for ALL to public using (
  (
    select
      auth.uid()
  ) = user_id
